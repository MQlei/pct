<template>
  <div class="breadcrumb">
    ...
    <el-breadcrumb>
      <el-breadcrumb-item v-for="(item, index) in breadcrumbItems" :key="index" @click="itemclick(item)">
        <template #separator>
          <el-icon>
            <ArrowRightBold />
          </el-icon>
        ...
        <span v-if="index < breadcrumbItems.length - 1" :to="item.path" :class="{ 'first-item': index === 0 }">
          {{ item.title }}
        </span>
